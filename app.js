document.addEventListener('DOMContentLoaded', () => {
    ...
    const videoEntity1 = document.querySelector('a-marker[url="marcador.patt"] a-video');
    const videoEntity2 = document.querySelector('a-marker[url="marcador2.patt"] a-video');
    const videoEntity3 = document.querySelector('a-marker[url="marcador3.patt"] a-video');
    ...
    startButton.addEventListener('click', () => {
        ...
        [videoPizza1, videoPizza2, videoPizza3].forEach(video => {
            if (video.paused) {
                video.play().catch(e => console.error("Error al desbloquear el video:", e));
    ...
    function updateVideoScaleAndPosition() {
        ...
        // Establece el ancho, alto y profundidad del video
        const aspectRatio = 16 / 9;
        const newScaleX = currentScale;
        const newScaleY = currentScale / aspectRatio;
        const newPositionZ = -currentScale * 0.5;
        const newPositionY = currentScale / 2; // Ajusta el video para que se centre

        activeVideoEntity.setAttribute('scale', `${newScaleX} ${newScaleY} 1`);
        activeVideoEntity.setAttribute('position', `0 ${newPositionY} ${newPositionZ}`);
